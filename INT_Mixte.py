...
class NetworkGenerator:
    def __init__(self, nD: int, nV: int, nM: int, nF: int, pGF: int, min_capacity: int, max_capacity: int):
        ...
        G = nx.barabasi_albert_graph(nD, 2)
        #D = list(G.nodes)
...
class Compact_Formulation_Mixte:
	...
	def optimize(self):
	    ...
	    if solution is None:
	        ...
	        # saving the data 
	        Sol_data = [spatial, temporal, collected, Flow_Path_dict, self.inst.shortest_path, self.inst.V_d]
...
if __name__ == "__main__":
    if len(sys.argv) != 8:
        print('Usage: python3.7 INT_Mixte.py [number nodes] [number items] [number monitoring application] [number flows] [percentage of given flows] [min_capacity] [max_capacity]')
        sys.exit(1)
    #network = sys.argv[1]
    nD = int(sys.argv[1])
    nV = int(sys.argv[2])
    ...
    Sol_Path = "./Solution/" + str(inst.nD) + "_" + str(inst.nV) + "_" + str(inst.nM) + "_" + str(inst.min_capacity) + "_" + str(inst.max_capacity) + "_" + str(seed)
    if not os.path.exists(Sol_Path):
        os.makedirs(Sol_Path)
    
    #saving the solution information
    sol = Sol_Path + "/" + "Sol_INT_Mixte_" + str(inst.nD) + "_" + str(inst.pGF) + "_" + str(inst.max_route) + ".txt"
